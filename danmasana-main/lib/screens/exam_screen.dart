...
class _ExamScreenState extends State<ExamScreen> {
  ...
  Future<void> _fetchExamProviders() async {
    final response = await https.get(Uri.parse('https://app.mikirudata.com.ng/api/exams/exams'));
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        examData = data.map((item) => {
          'name': item['name'],
          'price': double.parse(item['amount'].toString()), // Parsing price from amount
  ...
  void _onExamNameChanged(String? newValue) {
    setState(() {
      selectedExamId = examData.firstWhere((exam) => exam['name'] == newValue, orElse: () => {'id': null})['id'] as String?;
      // Find the price for the selected exam
      var selectedExam = examData.firstWhere((exam) => exam['name'] == newValue, orElse: () => {'price': 0.0});
 ...
 Future<void> _validateTransactionPin(String pin) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final errorMessage = errorData['message'] ?? 'An unexpected error occurred';
      _showFailedDialog(errorMessage); // Pass the server message to the failed dialog
    }
  } catch (e) {
    ...
    pdf.addPage(
      pw.Page(
        build: (pw.Context context) => pw.Center(
          child: pw.Text('Your Token: \n\n Exam ID: $selectedExamId\n Quantity: ${quantityController.text}\n Total Amount: ₦${amount.toStringAsFixed(2)}'),
        ),
      ),
    );

    await Printing.sharePdf(bytes: await pdf.save(), filename: 'exam_token.pdf');
  }
