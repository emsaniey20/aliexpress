...
class _TVCableScreenState extends State<TVCableScreen> {
  ...
  Future<void> _fetchCableProviders() async {
    try {
      final response = await https.get(Uri.parse('https://app.mikirudata.com.ng/api/cable/networks'));
      if (response.statusCode == 200) {
        setState(() {
          cableProviders = List<Map<String, dynamic>>.from(json.decode(response.body));
        });
      } else {
  ...
  Future<void> _fetchSubscriptions(String providerId) async {
    try {
      final response = await https.get(Uri.parse('https://app.mikirudata.com.ng/api/cable/plans/network/$providerId'));
      if (response.statusCode == 200) {
        setState(() {
          subscriptions = List<Map<String, dynamic>>.from(json.decode(response.body));
        });
      } else {
 ...
 Future<void> _validateIUCNumber(String iucNumber, String cableCode) async {
  ...
  if (selectedProvider['id'] == '0') {
    _showAlert(context, AlertType.error, 'Error', 'Selected provider not found.');
    return;
  }

  final networkId = selectedProvider['id'].toString();

  final url = Uri.parse('https://bankauradatasub.com/api/cable/cable-validation?iuc=$iucNumber&network_id=$networkId');
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['success']) {
        ...
        _showAlert(context, AlertType.error, 'Error', 'Validation failed: ${data['message']}');
      }
    } else {
      _showAlert(context, AlertType.error, 'Error', 'Failed to validate IUC number.');
    }
  } catch (e) {
 ...
 void _onProceed() {
  if (selectedCableProvider != null &&
    ...
    _showAlert(context, AlertType.error, 'Error', 'Please fill in all required fields.');
  }
}
  ...
  Alert(
    ...
    content: Column(
      ...
      children: <Widget>[
        ...
        Text('Subscription Cost: \NGN${costController.text}',
          style: TextStyle(
            fontSize: 16,
    ...
    buttons: [
      DialogButton(
        onPressed: () {
          _validateTransactionPinForTV(pinController.text); // Validate pin when the button is pressed
        },
        child: Text(
          "Validate Pin",
...
Future<void> _validateTransactionPinForTV(String pin) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      _showAlert(context, AlertType.error, "Failed", "Pin validation failed. Please try again.");
    }
  } catch (e) {
...
Future<void> _submitTVSubscription() async {
  ...
  if (selectedProvider['id'] == '0' || selectedPlan['id'] == '0') {
    _showAlert(context, AlertType.error, 'Error', 'Invalid provider or subscription selected.');
    return;
  }
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData['status'] == 'success') {
        _showAlert(context, AlertType.success, 'Success', 'Subscription successful!');
      } else {
        final errorMessage = responseData['message'] ?? 'Failed to process subscription.';
  ...
  void _showAlert(BuildContext context, AlertType type, String title, String message) {
    Alert(
      context: context,
      type: type,
      title: title,
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              hint: Text('Select Cable Provider', style: TextStyle(color: Colors.blue[900])),
              items: cableProviders.map((provider) {
                return DropdownMenuItem<String>(
                  value: provider['id'].toString(),
                  child: Text(provider['name'], style: TextStyle(color: Colors.blue[900])),
                );
              }).toList(),
              onChanged: (newValue) {
                setState(() {
                  selectedCableProvider = newValue;
                  _fetchSubscriptions(newValue!); // Fetch subscriptions when provider changes
                });
              },
            ),
            SizedBox(height: 20),
            ...
            DropdownButtonFormField<String>(
              ...
              hint: Text('Select Subscription', style: TextStyle(color: Colors.blue[900])),
              items: subscriptions.map((subscription) {
                return DropdownMenuItem<String>(
                  value: subscription['id'].toString(),
                  child: Text(subscription['plan_name'], style: TextStyle(color: Colors.blue[900])),
                );
              }).toList(),
              onChanged: (newValue) {
                setState(() {
                  ...
                  final totalCost = selectedPlan['amount'] + selectedPlan['charges'];
