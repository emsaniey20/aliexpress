...
class _ElectricBillScreenState extends State<ElectricBillScreen> {
  ...
  List<Map<String, dynamic>> discoNames = []; // Initially empty, to be populated by API
  List<Map<String, dynamic>> meterTypes = []; // Initially empty, to be populated by API
  ...
  return discoNames.firstWhere(
    ...
    orElse: () => {} // Return an empty map or handle the case where the Disco is not found
  );
}
 ...
 Future<void> fetchDiscoNames() async {
  try {
    ...
    if (response.statusCode == 200) {
      setState(() {
        discoNames = List<Map<String, dynamic>>.from(json.decode(response.body));
      });
    } else {
...
Future<void> fetchMeterTypes(String discoId) async {
  ...
  try {
    final response = await https.get(
      Uri.parse('https://app.mikirudata.com.ng/api/electricbill/meter-types?discoId=$discoId'),
    );
    ...
    _showAlert('Error', 'Failed to load Meter types. Please try again later.', AlertType.error);
  } finally {
    setState(() {
...
Future<void> _submitBillPayment() async {
  final url = Uri.parse('https://app.mikirudata.com.ng/api/electricbill/purchase');
  ...
  if (selectedDisco == null || selectedMeterType == null || meterNumberController.text.isEmpty) {
...
final requestBody = {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final amount = responseData['amount']; // Extract the amount paid from response
      ...
      final responseData = json.decode(response.body);
      final errorMessage = responseData['message'] ?? 'Failed to process bill payment. Please try again later.';
