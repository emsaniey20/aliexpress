...
class _DataPlanScreenState extends State<DataPlanScreen> {
  ...
  Future<void> fetchNetworkProviders() async {
    try {
      final response = await https.get(Uri.parse('https://app.mikirudata.com.ng/api/data/networks'));
  ...
  Future<void> fetchDataTypes() async {
    ...
    try {
      final response = await https.get(Uri.parse('https://app.mikirudata.com.ng/api/data/types?network=$selectedNetwork'));
 ...
 Future<void> fetchDataPlans() async {
  ...
  try {
    final response = await https.get(Uri.parse('https://app.mikirudata.com.ng/api/data/plans?type=$selectedDataType'));
...
Future<void> _submitDataPlan() async {
  final url = Uri.parse('https://app.mikirudata.com.ng/api/dataplan/purchase');
  ...
  final requestBody = {
    ...
    'requestId': 'Data_${DateTime.now().millisecondsSinceEpoch}', // Example requestId
    'identifier_number': phoneController.text, // Phone number as the identifier
  };
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final dataPlanName = selectedDataPlan!['name']; // Extract data plan name
      ...
      final responseData = json.decode(response.body);
      final errorMessage = responseData['message'] ?? 'Failed to purchase data plan. Please try again later.';
        ...
        child: Column(
          ...
          children: [
            ...
            CustomDropdown(
              ...
              onChanged: (newValue) {
                setState(() {
                  selectedNetwork = networkProviders.firstWhere(
            ...
            SizedBox(height: 20),
            CustomDropdown(
              value: selectedDataPlan != null
                  ? '${selectedDataPlan!['name']} - NGN ${selectedDataPlan!['amount']}'
                  : null,
              hint: 'Select Data Plan',
              items: dataPlans.map((plan) => '${plan['name']} - NGN ${plan['amount']}').toList(),
              onChanged: (newValue) {
                setState(() {
                  selectedDataPlan = dataPlans.firstWhere(
                    (plan) => '${plan['name']} - NGN ${plan['amount']}' == newValue,
                  );
                });
              },
            ),
            ...
            SizedBox(height: 20),
            Center( // Wrap ElevatedButton with Center
  ...
  Widget build(BuildContext context) {
    return DropdownButtonFormField<String>(
      ...
      decoration: InputDecoration(
        ...
        enabledBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(8.0),
          borderSide: BorderSide(color: Colors.blue[900]!), // Border color when enabled
        ),
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(8.0),
      ...
      style: TextStyle(color: Colors.blue[900]), // Dropdown selected item text color
      dropdownColor: Colors.white, // Background color of the dropdown container
      items: items.map((String item) {
        return DropdownMenuItem<String>(
          value: item,
          child: Container(
            width: double.infinity, // Ensures the dropdown item width is the same as the dropdown button
            child: Text(item),
          ),
        );
      }).toList(),
